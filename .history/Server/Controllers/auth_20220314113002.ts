 ...
 */
 ...
 export async function ProcessSignUp(req:Request, res:Response, next:NextFunction)
 {
         ...
         try {
             const newPeople = new People({email, password});   // Use the email and password to create a new User
             // Save the user to the database
             await newPeople.save();    // This is asynchronous! must use async (in the function) and await
             
             const token = jwt.sign({ userId: newPeople._id }, process.env.SECRET); // Secret key is hardcoded for now
             res.send({ token });
         }
         catch (err){
